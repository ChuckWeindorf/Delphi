...
uses
  ...
  unaVCIDE, unaDspControls, unaClasses, unaTypes, unaUtils, pmScriptWaveShare;
...
If fileexists(extractfilepath(Application.ExeName) + 'ScriptWaveAccount_O.cry') then
...
lstImportFiles.Directory:=edtScanFolder.text;
                ...
                If comScriptWaveComm.getfile(vstrAppBaseFolder +
                  ...
                  grdAlerts.cells[1,grdAlerts.rowcount-1] :=
                     'ScriptWavePhysicianTransfer version: ' +
                                 copy(vstlDir[vintLoop], cintFTPDirOffset+27,3);
                  lblFileTransferName.caption := '';
                  end;
                end;
          end;
...
If (pagMain.activePage = tabDictate) and (not vbolImmediateCommunicate) then
                                                                           exit;
{----------------}
...
If treMessages.Selected.Level = 1 then
    begin
    if treMessages.Selected.Parent.Text = 'Sent' then
         begin
...
begin
  ...
  if (not waveIn.active) then
    guiMessageBox(handle, waveIn.waveErrorAsString,
                  'Unable to open recording device', MB_OK or MB_ICONERROR)
  else
    begin
...
If f_memBlock = nil then
  ...
  vintAudioLevel:= 0;
  vintTestTime:= 0;
  waveIn.addConsumer(c_fftBand.fft);
...
begin
If (directoryexists(edtScanFolder.text)) and
                                           (length(edtScanFolder.text) > 2) then
  begin
  If copy(edtScanFolder.text,length(edtScanFolder.text),1) <> '\' then
     edtScanFolder.text := edtScanFolder.text + '\';
  vstrTransmitFolder:= edtScanFolder.text;
  comScriptWaveComm.SaveDefaults;
  lstImportFiles.Directory := vstrTransmitFolder;
  lstImportFiles.Refresh;
  end;
end;
